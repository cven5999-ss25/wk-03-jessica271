...
3.  Do not display (ignore) the outliers in the plot. **Note: Use a search engine or an AI tool to find the solution and add the link to the solution you have found.**
    ...
    <https://www.perplexity.ai/search/explain-the-following-boxplot-byqfR9_wRT2uAoz_EXS6Pg?login-source=imageGenerationInThread&login-new=false>
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    The barplot used rectangles to represent the x (continents) and y (count) axis while geo_bar represents the counties with their frequency hence producing identical plots.
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    They both use bars to represent frequencies.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe what you observe here:
    ...
    The breaks provide clear intervals in Life expectancy data on x axis, so as to control the number of bars in the histogram. For example, the first bar will represent 40 years while the next one will be 42.5 years.
...
5.  Describe in your own words what the `facet_wrap()` function does
    ...
    It splits data into subplots. For example, it split the plot from one to 5 subplots in reference to the continents.
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`
    ...
    ?country_colors - It outlines gapminder color schemes

    country_colors - It outlines all the countries in the data
...
```         
 git config --global user.email "Jessica.Maina@colorado.edu"
  git config --global user.name "jessica271"
```
